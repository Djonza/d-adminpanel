...
local function logAdminDutyStart(playerId)
    ...
    TriggerClientEvent('d-adminpanel:client:setAdminDutyStatus', playerId, true)
...
local function logAdminDutyEnd(playerId)
    ...
    TriggerClientEvent('d-adminpanel:client:setAdminDutyStatus', playerId, false)
    TriggerClientEvent('ox_lib:notify', playerId, {
    ...
    for _, group in ipairs(Config.AdminGroups) do
        if xPlayer.getGroup() == group then
            if adminDutyPlayers[source] then
                ...
                local message = string.format("%s (%s) je izasao na admin duznost", adminGroup, adminName)
                TriggerClientEvent("d-adminpanel:addLog", -1, "ADMIN", message)
                TriggerClientEvent('bubitag:ugasitag', source)
                TriggerClientEvent('bubi-id:toggle', source, false)
            else
                logAdminDutyStart(source)
                local message = string.format("%s (%s) je usao na admin duznost", adminGroup, adminName)
                TriggerClientEvent("d-adminpanel:addLog", -1, "ADMIN", message)
                TriggerClientEvent('bAdmin:showIDs', src)
                TriggerClientEvent('bubitag:upalitag', source)
                TriggerClientEvent('bubi-id:toggle', source, true)
            end
            return
        end
    end
...
RegisterCommand('adminlist', function(source)
    local xPlayer = ESX.GetPlayerFromId(source)
